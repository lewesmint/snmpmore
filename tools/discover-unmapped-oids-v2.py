...
# Pattern to match numeric OID at start of line
NUMERIC_OID_RE = re.compile(r"^(\.[\d.]+)")
# Pattern to match symbolic OID at start of line
...
class ObjectTypeInfo:
    ...
    is_complete: bool  # True if symbolic has no numeric arcs
...
def has_numeric_arcs(symbolic_oid: str) -> tuple[bool, str]:
    ...
    Returns:
        (has_numeric, reason) - whether it has numeric arcs and why
    """
...
def check_object_type(numeric_oid: str, mibdirs: str | None) -> bool:
    ...
    definition. Stub MIBs may define branch nodes as OBJECT-TYPE but
    without a SYNTAX clause - those are not real leaf nodes.
    """
    ...
    # Must have both OBJECT-TYPE and SYNTAX to be a real leaf
...
def find_object_type(
    ...
    # First, find if there's ANY OBJECT-TYPE in this OID path
    ...
    # Find the SHORTEST prefix that's still an OBJECT-TYPE
...
def analyse_single_oid(oid: str, mibdirs: str | None) -> None:
    ...
    if leaf_oid is None:
        ...
        deepest_num, deepest_sym, next_arc = find_deepest_resolved(
            numeric_oid, mibdirs
        )
...
def interactive_mode(mibdirs: str | None) -> None:
    ...
    print()
    print("Supports both numeric and symbolic OID formats:")
    print("  .1.3.6.1.2.1.1.1.0 = STRING: ...")
    print("  .iso.org.dod.internet.mgmt.mib-2.system.sysDescr.0 = STRING: ...")
    print()
    ...
    while True:
        ...
        # Find OBJECT-TYPE
        leaf_oid, leaf_len, is_complete, reason = find_object_type(
            numeric_oid, mibdirs
        )

        if leaf_oid is None:
            print(f"  ✗ UNRESOLVED: No OBJECT-TYPE found")
            deepest_num, deepest_sym, next_arc = find_deepest_resolved(
                numeric_oid, mibdirs
            )
        ...
        if is_complete:
            ...
            print(f"    Leaf OID: {leaf_oid}")
            print(f"    Leaf symbolic: {leaf_symbolic}")
            complete_count += 1
        else:
    ...
    total = complete_count + incomplete_count + unresolved_count
    if total > 0:
...
def do_snmpwalk(
    ...
    print(f"  Base OID: {base_oid}", file=sys.stderr)
...
def write_unresolved_file(
    ...
    with path.open("w", encoding="utf-8") as f:
        ...
        f.write("#\n")
        f.write("# Group by where MIB resolution stops.\n")
        f.write("# Use the 'Last resolved' OID to identify which MIB you need.\n")
        f.write("#\n\n")
        ...
        for sp, oid_list in sorted(
            ...
            f.write("\n")
            f.write("# To find the MIB, search for the symbolic name or numeric OID:\n")
            f.write(f"#   snmptranslate -M +YOUR_MIB_DIR -m ALL {sp.deepest_resolved_numeric}.{sp.next_arc}\n")
            f.write("#   Or search: https://oidref.com/ or https://www.oid-info.com/\n")
            f.write("\n")
...
def write_log_file(
    ...
    with log_path.open("w", encoding="utf-8") as f:
        ...
        complete_types = [
            info for info in results.object_types.values() if info.is_complete
        ]
        incomplete_types = [
            info for info in results.object_types.values() if not info.is_complete
        ]
        ...
        if incomplete_types:
            f.write("=" * 70 + "\n")
            f.write("INCOMPLETE OBJECT-TYPEs (STUB MIBs)\n")
            f.write("=" * 70 + "\n\n")

            incomplete_types.sort(key=lambda x: x.instance_count, reverse=True)

            for info in incomplete_types:
                f.write(f"OBJECT-TYPE: {info.numeric_oid}\n")
                if info.symbolic_name:
                    f.write(f"  Symbolic: {info.symbolic_name}\n")
                f.write(f"  Instance count: {info.instance_count}\n")
                f.write("\n")

        complex_types = [
            (oid, info) for oid, info in results.object_types.items()
            if info.is_complete and any(length > 1 for length in info.index_lengths)
        ]

        if complex_types:
            f.write("=" * 70 + "\n")
            f.write("COMPLETE OBJECT-TYPEs WITH COMPLEX INDICES\n")
            f.write("=" * 70 + "\n\n")

            complex_types.sort(key=lambda x: x[1].instance_count, reverse=True)

            for leaf_oid, info in complex_types:
                f.write(f"OBJECT-TYPE: {leaf_oid}\n")
                if info.symbolic_name:
                    f.write(f"  Symbolic: {info.symbolic_name}\n")
                f.write(f"  Instance count: {info.instance_count}\n")
                f.write("  Index lengths: ")
                f.write(", ".join(
                    f"{length} arcs ({count}x)"
                    for length, count in sorted(info.index_lengths.items())
                ))
                f.write("\n")

                if info.example_indices:
                    f.write("  Example indices:\n")
                    for idx_arcs in info.example_indices:
                        f.write(f"    [{', '.join(idx_arcs)}]\n")
                f.write("\n")

        f.write("=" * 70 + "\n")
        f.write("ALL OBJECT-TYPEs SUMMARY\n")
        f.write("=" * 70 + "\n\n")

        for leaf_oid in sorted(
            results.object_types.keys(),
            key=lambda x: [int(p) for p in split_oid(x)],
        ):
            info = results.object_types[leaf_oid]
            name = info.symbolic_name or "(unknown)"
            short_name = name.split(".")[-1] if "." in name else name
            status = "✓" if info.is_complete else "✗"
            f.write(
                f"{status} {leaf_oid}  {short_name}  "
                f"instances={info.instance_count}\n"
            )
        ...
        if results.unresolved:
            f.write("=" * 70 + "\n")
            f.write("UNRESOLVED OIDs (NO OBJECT-TYPE FOUND)\n")
            f.write("=" * 70 + "\n\n")
            ...
            for sp, oid_list in sorted(
                ...
                f.write(f"Stop point: {sp.deepest_resolved_numeric}\n")
                f.write(f"  Symbolic: {sp.deepest_resolved_symbolic}\n")
                f.write(f"  Missing child arc: {sp.next_arc}\n")
                f.write(f"  Affected OIDs: {len(oid_list)}\n")
                for oid in oid_list[:5]:
                    f.write(f"    {oid}\n")
                if len(oid_list) > 5:
                    f.write(f"    ... and {len(oid_list) - 5} more\n")
                f.write("\n")
...
def print_summary(results: AnalysisResults) -> None:
    ...
    total_instances = sum(
        info.instance_count for info in results.object_types.values()
    )
    complete_instances = sum(
        info.instance_count
        for info in results.object_types.values()
        if info.is_complete
    )
    ...
    print(f"  Unique OBJECT-TYPEs:       {len(complete_types)}")
    print()
    print(f"Incomplete MIB (stub) OIDs:  {incomplete_instances}")
    print(f"  Unique OBJECT-TYPEs:       {len(incomplete_types)}")
    print()
    print(f"Unresolved OIDs:             {total_unresolved}")
    print()

    if total > 0:
        full_coverage = (complete_instances * 100) / total
        partial_coverage = ((complete_instances + incomplete_instances) * 100) / total
        print(f"Full MIB Coverage:    {full_coverage:.1f}%")
        print(f"Partial Coverage:     {partial_coverage:.1f}%")
        print()

    if incomplete_instances == 0 and total_unresolved == 0:
        print("✓ SUCCESS: All OIDs fully resolved with complete MIBs.")
        return

    if incomplete_types:
        print("⚠ INCOMPLETE MIBs DETECTED (stub definitions)")
        print()

        families: Counter[str] = Counter()
        for info in incomplete_types:
            families[bucket_from_oid(info.numeric_oid)] += info.instance_count

        print("Affected OIDs by family:")
        for family, count in families.most_common():
            print(f"  {family}: {count}")
        print()

        print("Incomplete OBJECT-TYPEs (top 10):")
        incomplete_types.sort(key=lambda x: x.instance_count, reverse=True)
        for info in incomplete_types[:10]:
            print(f"  {info.numeric_oid}")
            if info.symbolic_name:
                print(f"    Symbolic: {info.symbolic_name}")
            print(f"    Instances: {info.instance_count}")
        print()

    if results.unresolved:
        print("✗ MISSING MIBs DETECTED (no OBJECT-TYPE found)")
        print()

        unresolved_families: Counter[str] = Counter()
        for oid, _ in results.unresolved:
            unresolved_families[bucket_from_oid(oid)] += 1

        print("Unresolved OIDs by family:")
        for family, count in unresolved_families.most_common():
            print(f"  {family}: {count}")
        print()

        stop_points: Counter[StopPoint] = Counter()
        for _, sp in results.unresolved:
            stop_points[sp] += 1

        print("Where MIB resolution stops (top 10):")
        for sp, count in stop_points.most_common(10):
            print(f"\n  Count: {count}")
            print(f"  Last resolved: {sp.deepest_resolved_numeric}")
            print(f"  Symbolic: {sp.deepest_resolved_symbolic}")
            print(f"  Missing child: {sp.next_arc}")


def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Perform an SNMP walk and analyse MIB coverage. "
            "Identifies missing and incomplete MIBs."
        ),
...
Examples:
  ...
  # Interactive mode - paste walk output
  %(prog)s --interactive

  # Analyse a single OID
  %(prog)s --oid .1.3.6.1.2.1.1.1.0

  # Walk a different host
  %(prog)s --host 10.0.0.1

  # Verify incomplete detections
  %(prog)s --verify
""",
    )
    ...
    parser.add_argument(
        ...
        help=f"Target device IP/hostname (default: {DEFAULT_HOST})",
    )
    parser.add_argument(
        "--community",
    ...
    parser.add_argument(
        ...
        choices=["1", "2c", "3"],
        help=f"SNMP version (default: {DEFAULT_VERSION})",
    )
    parser.add_argument(
        "--base-oid",
        default=DEFAULT_BASE_OID,
        help=f"Base OID for walk (default: {DEFAULT_BASE_OID})",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        "--mibdirs",
        default=None,
        help="MIB search directories for snmptranslate -M",
    )
    parser.add_argument(
        "--max-oids",
        type=int,
        default=None,
        help="Limit number of OIDs to analyse",
    )
    parser.add_argument(
        "--log-file",
        type=Path,
        default=None,
        help="Write detailed analysis log to file",
    )
    parser.add_argument(
        "--unresolved-file",
        type=Path,
        default=None,
        help="Write unresolved OIDs to file (for finding missing MIBs)",
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
        help="Show detailed per-OID resolution info",
    )
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Verify incomplete detections",
    )
    parser.add_argument(
        "-i", "--interactive",
        action="store_true",
        help="Interactive mode - paste walk output for analysis",
    )
    parser.add_argument(
        "--oid",
        default=None,
        help="Analyse a single OID (detailed output)",
    )
    ...
    print("\n" + "=" * 60, file=sys.stderr)
    obj_info = check_object_type.cache_info()
    sym_info = translate_to_symbolic.cache_info()
    obj_total = obj_info.hits + obj_info.misses
    sym_total = sym_info.hits + sym_info.misses
    print(
        f"Cache: check_object_type {obj_info.hits}/{obj_total} hits "
        f"({obj_info.hits * 100 // obj_total if obj_total else 0}%), "
        f"translate_to_symbolic {sym_info.hits}/{sym_total} hits "
        f"({sym_info.hits * 100 // sym_total if sym_total else 0}%)",
        file=sys.stderr,
    )
